...
/**
     ...
     */
export default function FrutaScreen({ navigation }) {
    ...
    return (
        ...
        <Text style={{ color: "white", fontSize: 23, marginBottom: 30}}>Pesquise o nome da fruta:</Text>
        <TextInput 
            label="Buscar"
        ...
        <View style={{ alignContent: "center",marginTop: 30, border:"2px solid #FF4340", padding: 20, borderRadius: 3}}>
            ...
            renderItem={({item}) => (
                ...
                <Text style={{ color: "white", fontSize: 15}}>{`\u2022 ${item.gosto}`}</Text>
                <Text style={{ color: "white", fontSize: 15}}>{`\u2022 ${item.cor}`}</Text>
                </View>
            )}
            key={(item) => item.id}
            />
